...
/*   Updated: 2025/02/08 15:00:17 by juduchar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init_color_sets(t_data *data)
{
	...
	nb_color_sets = 9;
	data->colors = (int *)ft_calloc(nb_color_sets, sizeof(int));
	if (!data->colors)
		return (0);
...
int	ft_init_opposite_color_sets(t_data *data, int *colors, int nb_color_sets)
{
	int	i;

	data->opposite_colors = (int *)ft_calloc(nb_color_sets, sizeof(int));
	if (!data->opposite_colors)
		return (0);
	i = 0;
	while (i < nb_color_sets)
	{
		data->opposite_colors[i] = ft_get_opposite_color(colors[i]);
		i++;
	}
	return (1);
}
